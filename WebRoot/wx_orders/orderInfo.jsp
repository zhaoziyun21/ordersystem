    ...
    <script>
		...
		$(function(){
			...
			$(".address img").click(function(){
				window.location.href="${root}order/getAddressListWxXFoodSendAddress.do?orderInfo="+orderInfoNew+"&vistorName="+vistorNameNew+"&type="+typeNew+"&leadId="+leadIdNew+"&infos="+infosNew+"&orderType="+orderTypeNew+"&foodBusiness="+foodBusinessNew+"&checkedBusiness="+checkedBusiness;
			})
		...
		function dc_food(){
            ...
            for(var i=0;i<ul.length;i++){
	            ...
	            orderInfo.price = $(ul[i]).find('.car-num .sell_price').val();//套餐
			...
			$.ajax({
			    url:"${root}/wechat/buyFoodWeChat.do",    //请求的url地址
...
#consignee_infor .address img {
    ...
    <div>
		...
		<c:choose>
       		...
       		<c:otherwise>
       			<div style="width:100%">
					<h3 style="display: block;width: 15.96rem;margin-left: 1.28rem;color: red;font-size: 0.7rem;margin-top: 0.8rem;">本餐饮公司不支持派送，需要凭证饭票到店取餐！</h3>
				</div>
       		</c:otherwise>
       	</c:choose>
