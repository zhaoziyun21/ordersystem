...
<html>
  ...
  <script>
	var grid1=null;
	var parmsData =[{name: 'Q|xfsr.del_flag|S|EQ', value:'0'}];
	function initGrids(parmsData){
		grid1 = $(".table_box").ligerGrid({
			url: '${root}order/getRegionListXFoodSendRegion.do',
			parms:parmsData,
			pageSize:10,
			//height:'99%',	
			...
			columns: [
				{ display: '派送范围名称', width:'30%', name: 'region_name' },
				{ display: '描述', width:'40%', name: 'region_desc' },
	...
	function add(){
		$.ligerDialog.open({ 
            height: 350,
            url: '${root}order/toAddRegionXFoodSendRegion.do',
            width: 500,
            name:'view',
            title:'新增派送范围',
            isResize:true	
	...
	function del(id,del_flag){
		...
		if(del_flag==0){
			p="停用";
			parmsData =[{name: 'Q|xfsr.del_flag|S|EQ', value:'0'}];
		}else if(del_flag==1){
			p="启用";
			parmsData =[{name: 'Q|xfsr.del_flag|S|EQ', value:'1'}];
		}
		
	    $.ligerDialog.confirm('确定'+p+'该《派送范围》吗？', function (yes){
          	  if(yes){
                $.ajax({
              	   url:'${root}order/delRegionXFoodSendRegion.do',
              	   type:'post',
              	   data:{'regionId':id},
              	   dataType:'html',
              	   async:false,
	...
	function view1(id){
		$.ligerDialog.open({ 
	        height: 350,
	        url: '${root}order/editRegionXFoodSendRegion.do?regionId='+id,
	        width: 500,
	        name:'view',
	        title:'编辑派送范围',
	        isResize:true
  ...
  <body>
  <form id="searchBox"> 
	  <input type="radio" name="Q|xfsr.del_flag|S|EQ" value="0" checked="checked"/>启用
	  <input type="radio" name="Q|xfsr.del_flag|S|EQ" value="1"/>禁用
	  <input type="text" id="str" name="str" value="" placeholder="请输入派送范围名称"/>
